...
export class AppComponent implements OnInit {
  ...
  validationEmail(value: boolean) {
    const input = document.querySelector('.gmail input') as HTMLInputElement | null;
    const emailIcon = document.querySelector('.email-icon') as HTMLElement | null;

    if (!input || !emailIcon) {
      console.error("Input or Email icon element not found.");
      return;
    }
  ...
  loadEmployees() {
    this.employeeService.getAllEmployees(this.currentPage, this.itemsPerPage).subscribe((data: any) => {
      // data.map((c: any) => { c.createdDate = c.createdDate.split("T"), c.createdDate = c.createdDate[0] })
      this.employees = data;
      // data.map((c: any) => { console.log(typeof c.createdDate.split("T"), c.createdDate = c.createdDate[0]) })
    });
  }
  ...
  loadAllEmployees() {
    this.employeeService.getAllWithoutPaginationEmployees().subscribe((data: any) => {
      ...
      // this.allEmployeesWithoutPagination.map((c) => { this.allEmployeesWithoutPagination = c.createdDate.split("T")[0] })
      console.log(data)
      // console.log(this.allEmployeesWithoutPagination)
      // console.log(this.allEmployeesWithoutPagination, "after change")

    });
    // console.log("data all emp weithout psagination",this.allEmployeesWithoutPagination)
  }
  createEmployee() {
  ...
  editEmployee(email: string) {
    ...
    this.email = email;
    console.log("edit emp", this.allEmployeesWithoutPagination)
    // console.log("email",email)
    const objectsWithEmail = this.allEmployeesWithoutPagination.find(obj => obj.email == this.email);
